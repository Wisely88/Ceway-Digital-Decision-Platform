...
from multiregime_v27 import MULTIREGIME_V27_VERSION, review_scarcity_ranking  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Review a frozen V2.7 scarcity ranking without regeneration")
    ...
    if plan.get("generator_version") != MULTIREGIME_V27_VERSION:
        raise ValueError("Review only accepts a frozen V2.7 plan")
    actual_front, actual_back = load_actual(args.game, args.issue)
    result = review_scarcity_ranking(plan, actual_front, actual_back)
