...
def run_game(
    *,
    ...
    if game == "DLT":
        ...
        candidate_builder = generate_dlt_coverage_single
        ...
        candidate_builder = generate_ssq_coverage_single
    ...
    candidate_vs_legacy_best = []
    candidate_vs_random_best = []
    candidate_vs_legacy_ticket = []
    candidate_vs_random_ticket = []
    candidate_vs_legacy_record = []
    ...
    for index in range(start_index, end_index + 1):
        ...
        candidate_plan = candidate_builder(
            budget,
            score_table,
            back_scores,
            strategy,
        ...
        legacy_best_value = best_hit_units(legacy_review)
        ...
        legacy_ticket_value = mean_ticket_hit_units(legacy_review)
        ...
        legacy_record_value = record_hit(legacy_review)
        candidate_record_value = record_hit(candidate_review)
        random_record_value = mean(record_hit(item) for item in random_reviews)

        candidate_vs_legacy_best.append(candidate_best_value - legacy_best_value)
        candidate_vs_random_best.append(candidate_best_value - random_best_value)
        candidate_vs_legacy_ticket.append(candidate_ticket_value - legacy_ticket_value)
        candidate_vs_random_ticket.append(candidate_ticket_value - random_ticket_value)
        candidate_vs_legacy_record.append(candidate_record_value - legacy_record_value)
        ...
        legacy_best.append(legacy_best_value)
        candidate_best.append(candidate_best_value)
        random_best.append(random_best_value)

    legacy_best_ci = ci(candidate_vs_legacy_best, f"{game}-{window}-candidate-vs-legacy-best")
    random_best_ci = ci(candidate_vs_random_best, f"{game}-{window}-candidate-vs-random-best")
    metrics = {
        "candidate_vs_legacy_best_hit": legacy_best_ci,
        "candidate_vs_random_best_hit": random_best_ci,
        "candidate_vs_legacy_mean_ticket_hit": ci(
            candidate_vs_legacy_ticket, f"{game}-{window}-candidate-vs-legacy-ticket"
        ),
        "candidate_vs_random_mean_ticket_hit": ci(
            candidate_vs_random_ticket, f"{game}-{window}-candidate-vs-random-ticket"
        ),
        "candidate_vs_legacy_record_hit": ci(
            candidate_vs_legacy_record, f"{game}-{window}-candidate-vs-legacy-record"
        ),
        "candidate_vs_random_record_hit": ci(
            candidate_vs_random_record, f"{game}-{window}-candidate-vs-random-record"
        ),
        "legacy_mean_best_hits": round(mean(legacy_best), 4),
    ...
    if legacy_best_ci["ci95_low"] > 0 and random_best_ci["ci95_low"] > 0:
        decision = "PROMOTE_CANDIDATE"
        reason = "coverage generator significantly beats legacy and matched random on best-hit metric"
    elif legacy_best_ci["ci95_low"] > 0 and random_best_ci["ci95_high"] >= 0:
        decision = "HOLD_FOR_HOLDOUT"
        reason = "coverage generator significantly beats legacy; matched-random comparison is non-negative/inconclusive"
    elif legacy_best_ci["ci95_high"] < 0:
        decision = "REJECT"
        reason = "coverage generator is significantly worse than legacy"
    else:
        decision = "HOLD"
    ...
    return {
        "game": game,
        "window": window,
        "periods": len(candidate_vs_legacy_best),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="A/B legacy vs V2 coverage-aware generator")
    ...
    parser.add_argument("--output", type=Path, default=ROOT_DIR / "artifacts" / "ceway_v2_generator_ablation.json")
    ...
    for game in ("DLT", "SSQ"):
        for window in windows:
            print(f"A/B {game}: periods={args.periods}, window={window}", flush=True)
            row = run_game(
                game=game,
    ...
    report = {
        "schema_version": "ceway.v2.generator-ablation.1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "sample_role": "development_diagnostic_not_final_holdout",
        "settings": {
            "periods": args.periods,
            "windows": windows,
            "baseline_seeds": args.baseline_seeds,
        ...
        "rows": rows,
        "interpretation": (
            "This run uses the same recent development window that diagnosed coverage concentration. "
            "Even a positive result must be validated on a non-overlapping holdout before production promotion."
        ),
    }
