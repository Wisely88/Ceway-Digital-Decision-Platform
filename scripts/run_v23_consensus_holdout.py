...
from consensus_v23 import CONSENSUS_V23_VERSION  # noqa: E402
from engine import load_dlt_history, load_ssq_history  # noqa: E402
from scripts.run_v23_consensus_ablation import run_game  # noqa: E402


FRESH_V23_EXCLUDED_RECENT_POINTS = 150
...
def holdout_history(history: list[dict], excluded_recent_points: int) -> list[dict]:
    ...
    if len(history) <= excluded_recent_points + 32:
        raise ValueError("not enough history for a non-overlapping V2.3 holdout block")
...
def block_provenance(history: list[dict], periods: int, excluded_recent_points: int) -> dict:
    ...
    if excluded_recent_points < periods * 3:
        raise ValueError(
            "fresh V2.3 holdout must exclude three consumed blocks: development + V1 holdout + V2.1 holdout"
        )
    if len(history) < excluded_recent_points + periods + 1:
        raise ValueError("not enough history to describe V2.3 holdout boundaries")
    ...
    return {
        ...
        "v23_fresh_holdout_block": outcome_block(n - excluded_recent_points, periods),
        "v22_consumed_this_block": False,
        "blocks_overlap": False,
        "rule": (
            "V2.3 fresh holdout outcomes are older than development and both consumed V1/V2.1 holdouts. "
            "V2.2's gated holdout was skipped, so this block remains untouched."
        ),
    }
...
def fresh_holdout_gate(game: str, metrics: dict) -> dict:
    ...
    if structural_ok and legacy_positive_all and legacy_significant_two and v21_nonnegative_two and random_not_significantly_worse:
        ...
        reason = "V2.3 passes the pre-registered fresh holdout gate"
    elif not structural_ok:
        decision = "REJECT"
        reason = "V2.3 misses the pre-registered structural diversity target"
    elif significant_legacy_loss:
        decision = "REJECT"
        reason = "V2.3 is significantly worse than legacy in at least one frozen baseline comparison"
    elif significant_random_loss:
        decision = "REJECT"
        reason = "V2.3 is significantly worse than structure-matched random"
    else:
        decision = "HOLD"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fresh historical holdout for CEWAY V2.3 median-rank consensus")
    parser.add_argument("--periods", type=int, default=50)
    parser.add_argument(
        "--excluded-recent-points",
        type=int,
        default=FRESH_V23_EXCLUDED_RECENT_POINTS,
        help="Default 150 = dev50 + V1 holdout50 + V2.1 holdout50; V2.2 did not consume this block.",
    )
    parser.add_argument("--baseline-seeds", type=int, default=3)
    parser.add_argument("--budget", type=int, default=20)
    parser.add_argument("--strategy", type=str, default="balanced")
    parser.add_argument("--output", type=Path, default=ROOT_DIR / "artifacts" / "ceway_v23_consensus_holdout.json")
    args = parser.parse_args()

    if args.excluded_recent_points < args.periods * 3:
        raise ValueError(
            "V2.3 fresh holdout refused: excluded_recent_points must cover dev + V1 holdout + V2.1 holdout"
        )
    ...
    for game in ("DLT", "SSQ"):
        print(
            f"V2.3 FRESH HOLDOUT {game}: periods={args.periods}, excluded_recent_points={args.excluded_recent_points}",
            flush=True,
        )
        row = run_game(
            game=game,
    ...
    report = {
        "schema_version": "ceway.v2.3.median-consensus-holdout.1",
        ...
        "interpretation": (
            "This block remains untouched because V2.2's holdout was gated off. Passing can authorize unchanged forward "
            "shadow validation, not production merge or predictive-certainty claims."
        ),
    }
