...
async def run_scene_flow(client: CdpClient, scene_name: str) -> dict:
    ...
    await wait_for_text(client, "投注方案")
    await assert_no_document_overflow(client, f"{scene_name}工作台")
    await click_text(client, "投注方案", ".side-nav-item")
    await wait_for_text(client, "生成系统建议")
    await click_text(client, "生成系统建议")
    await wait_for_text(client, "再生成一组")
    await click_text(client, "保存方案")
    await wait_for_text(client, "已保存")
    await click_text(client, "推荐复盘", ".side-nav-item")
    await wait_for_text(client, "推荐复盘")
    ...
    await wait_for_text(client, "场景选择页")
    return result
