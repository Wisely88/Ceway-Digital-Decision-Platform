...
def development_gate(game: str, metrics: dict) -> dict:
    ...
    if structural_ok and legacy_positive_all and legacy_significant_two and v21_nonnegative_two and random_not_significantly_worse:
        ...
        reason = "fixed multi-window consensus passes structural, legacy, V2.1, and matched-random development gates"
    elif not structural_ok:
        decision = "REJECT_STRUCTURE"
        reason = "consensus portfolio misses the pre-registered diversity target"
    elif metrics["candidate_vs_random_best_hit"]["ci95_high"] < 0:
        decision = "REJECT_RANDOM"
        reason = "consensus candidate is significantly worse than structure-matched random on development data"
    elif sum(row["candidate_vs_v21_best_hit"]["ci95_high"] < 0 for row in baseline_rows) >= 2:
        decision = "REJECT_V21"
        reason = "consensus candidate is significantly worse than frozen V2.1 in at least two baseline windows"
    else:
        decision = "HOLD"
        reason = "development evidence is mixed; do not consume the fresh V2.2 holdout"
...
def run_game(
    *,
    ...
    for index in range(start_index, end_index + 1):
        ...
        consensus_front = equal_rank_consensus(front_tables, windows=windows)
        consensus_back = equal_rank_consensus(back_tables, windows=windows)
        ...
        for seed_index in range(max(1, baseline_seeds)):
            random_plan = robust_structure_matched_random_plan(
                candidate_plan,
                spec,
                seed=f"v22-consensus-{game}-{source_issue}-{seed_index}",
        ...
        baseline_comparisons.append(
            {
                ...
                "candidate_vs_legacy_best_hit": ci(values["candidate_vs_legacy"], f"v22-{game}-{window}-legacy"),
                "candidate_vs_v21_best_hit": ci(values["candidate_vs_v21"], f"v22-{game}-{window}-v21"),
    ...
    metrics = {
        "candidate_vs_random_best_hit": ci(candidate_vs_random, f"v22-{game}-random-best"),
        "candidate_vs_random_mean_ticket_hit": ci(candidate_vs_random_ticket, f"v22-{game}-random-ticket"),
        "candidate_vs_random_record_hit": ci(candidate_vs_random_record, f"v22-{game}-random-record"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Development A/B for CEWAY V2.2 multi-window rank consensus")
    parser.add_argument("--periods", type=int, default=50)
    parser.add_argument("--baseline-seeds", type=int, default=3)
    parser.add_argument("--budget", type=int, default=20)
    parser.add_argument("--strategy", type=str, default="balanced")
    parser.add_argument("--output", type=Path, default=ROOT_DIR / "artifacts" / "ceway_v22_consensus_ablation.json")
    args = parser.parse_args()

    histories = {"DLT": load_dlt_history(), "SSQ": load_ssq_history()}
    rows = []
    for game in ("DLT", "SSQ"):
        print(f"V2.2 consensus development {game}: periods={args.periods}, windows={CONSENSUS_WINDOWS}", flush=True)
        row = run_game(
            game=game,
    ...
    report = {
        "schema_version": "ceway.v2.2.consensus-ablation.1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "sample_role": "development_only_reusing_consumed_recent_block",
        "candidate_generator_version": CONSENSUS_V22_VERSION,
        "parameter_provenance": (
            "Windows are fixed at 50/100/200 and combined as equal rank percentiles (1/3 each). "
            "No lottery outcomes were used to choose the weights. The V2.1 exposure combination engine is unchanged."
        ),
        "settings": {
            "periods": args.periods,
        ...
        "interpretation": (
            "This development run reuses the already-consumed recent 50 outcomes. It can select whether V2.2 is worth "
            "testing on a new older holdout, but it is not promotion evidence itself."
        ),
    }
