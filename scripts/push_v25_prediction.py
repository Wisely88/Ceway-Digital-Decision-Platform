...
from multiregime_v25 import DLT, SSQ, generate_multiregime_plan  # noqa: E402
from pushplus_v25 import build_v25_prediction_digest, send_pushplus  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render/send CEWAY V2.5 prediction using one shared PushPlus digest")
    ...
    plan = generate_multiregime_plan(history, spec, budget=args.budget, history_cutoff_issue=cutoff)
    digest = build_v25_prediction_digest(plan, target_issue=target)

    # This exact string is also assigned to PushPlus payload.content.
    print(digest["markdown"])
    if args.send:
        result = send_pushplus(digest)
        print(f"\nPushPlus: code={result.get('code')} msg={result.get('msg')}")
    else:
        print("\n[DRY-RUN] PushPlus not sent. Set PUSHPLUS_TOKEN and add --send to deliver this exact content.")
    return 0
