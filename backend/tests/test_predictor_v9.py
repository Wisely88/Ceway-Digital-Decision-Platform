...
def make_history(spec, count=120):
    ...
    for issue in range(1, count + 1):
        if spec.game == "DLT":
            front = sorted(set([((issue + offset * 3) % 35) + 1 for offset in range(5)]))
            back = sorted(set([((issue + offset * 5) % 12) + 1 for offset in range(2)]))
            while len(front) < 5:
                front = sorted(set(front + [len(front) + 1]))
            while len(back) < 2:
                back = sorted(set(back + [len(back) + 1]))
        else:
            front = sorted(set([((issue + offset * 3) % 33) + 1 for offset in range(6)]))
            back = [((issue * 2) % 16) + 1]
            while len(front) < 6:
                front = sorted(set(front + [len(front) + 1]))
        rows.append({
...
class PredictorV9Tests(unittest.TestCase):
    def test_score_table_is_complete(self):
        result = score_numbers_v9(make_history(DLT), DLT)
    ...
    def test_prediction_respects_budget_and_unique_tickets(self):
        plan = generate_prediction_v9(make_history(DLT), DLT, budget=20, seed="test")
    ...
    def test_ssq_prediction_shape(self):
        plan = generate_prediction_v9(make_history(SSQ), SSQ, budget=20, seed="test")
        self.assertEqual(plan["tickets"], 10)
    ...
    def test_freeze_is_deterministic(self):
        ...
        first = generate_prediction_v9(history, DLT, budget=20, seed="freeze", history_cutoff_issue="120")
        second = generate_prediction_v9(history, DLT, budget=20, seed="freeze", history_cutoff_issue="120")
        self.assertEqual(first["items"], second["items"])

    def test_cutoff_blocks_future_data(self):
        history = make_history(DLT)
        baseline = generate_prediction_v9(history[:90], DLT, budget=20, seed="cutoff", history_cutoff_issue="90")
        leaked_tail = generate_prediction_v9(history, DLT, budget=20, seed="cutoff", history_cutoff_issue="90")
