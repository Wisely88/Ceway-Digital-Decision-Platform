...
def _number_rows(history: Sequence[dict], spec: GameSpec, area: str, windows: Sequence[int]) -> list[dict]:
    ...
    for number in range(1, pool + 1):
        ...
        recency = _weighted_rate(
            history,
            number,
            area,
            min(max(windows), len(history)),
            half_life=max(6, min(24, max(windows) / 4)),
        )
    ...
    for item in raw:
        ...
        item["evidence_strength"] = round(
            _clamp((len(history) / max(windows)) * 0.7 + stability * 0.3),
            3,
        )
        item["explanation"] = (
...
def score_numbers_v9(
    history: Sequence[dict],
    spec: GameSpec,
    windows: Sequence[int] = (20, 50, 100, 200),
    ...
    usable_windows = tuple(
        sorted({min(int(window), len(history)) for window in windows if int(window) >= 10})
    )
...
def _portfolio_select(
    ...
    score_by_ticket: dict[tuple[int, ...], float],
    ticket_count: int,
    seed: str,
) -> list[tuple[int, ...]]:
    if not candidates:
    ...
    while remaining and len(selected) < ticket_count:
        ...
        for candidate in remaining:
            ...
            jaccard = max(
                (
                    len(candidate_set & set(item)) / len(candidate_set | set(item))
            ...
            pair_reuse = sum(
                pair_usage[tuple(sorted((left, right)))]
            ...
            jitter = rng.random() * 0.0001
            key = objective + jitter
...
def _diversity(numbers: Iterable[Iterable[int]]) -> dict:
    ...
    return {
        ...
        "mean_jaccard": round(mean(values), 4),
        "max_jaccard": round(max(values), 4),
    }
...
def generate_prediction_v9(
    history: Sequence[dict],
    spec: GameSpec,
    budget: int = 20,
    windows: Sequence[int] = (20, 50, 100, 200),
    seed: str | int = 42,
    candidate_band: int = 18,
    history_cutoff_issue: str | None = None,
) -> dict:
    if budget < 2:
        raise ValueError("budget 必须 >= 2")

    scored = score_numbers_v9(
        history,
        spec,
        windows,
    ...
    score_by_ticket = {
        candidate: _combination_score(candidate, front_rows, spec)
    ...
    if spec.bonus_pick == 1:
        ...
        back_candidates = list(
            combinations(ranked_back[: min(8, len(ranked_back))], spec.bonus_pick)
        )
        back_scores = {
            candidate: mean(back_rows[n]["total_score"] for n in candidate)
    ...
    return {
        ...
        "score_table": scored,
        "portfolio": {
            "front_diversity": _diversity(t["front"] for t in tickets),
            "back_diversity": _diversity(t["back"] for t in tickets),
            "candidate_band": len(candidate_numbers),
        },
        "research_guard": {
            "production_enabled": False,
            "claim": "descriptive ranking and portfolio construction; not a claim of lottery predictability",
        },
    }
